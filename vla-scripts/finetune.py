...
def parse_action_dims(action_dims: str) -> List[int]:
    dims = [int(dim.strip()) for dim in action_dims.split(",") if dim.strip()]
    if not dims or any(dim < 0 or dim >= 7 for dim in dims):
        raise ValueError(f"Action dimensions must be indices in [0, 6], got: {dims}")
...
def extract_action_features(last_hidden_state, labels, num_patches, action_token_begin_idx):
    aligned_hidden = last_hidden_state[:, num_patches:-1]
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    residual_action_dims = parse_action_dims(cfg.residual_action_dims)
    action_chunk_dims = parse_action_dims(cfg.action_chunk_dims)
    ...
    vla = OpenVLAForActionPrediction.from_pretrained(
        ...
        low_cpu_mem_usage=not use_auxiliary_heads,
    ...
    if cfg.use_lora:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=OPENVLA_LORA_TARGET_MODULES if use_auxiliary_heads else "all-linear",
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            action_logits = output.logits[:, metric_vla.vision_backbone.featurizer.patch_embed.num_patches : -1]
